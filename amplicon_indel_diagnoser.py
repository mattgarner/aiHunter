...
#                    slidevalue=slide(Seq(R2reads[i]).reverse_complement(), R1reads[i])
        ...
        print "Normal amplicon length = "+str(ampliconlength)
        print "Mutant amplicon length = "+str(mutantampliconlength)

        print "R1reads_w_primermatch", len(R1reads_w_primermatch)
        ...
        for i in range(0,len(R1reads_w_primermatch)):
            ...
            mutantindelsizeflag=hybridize(R2reads_w_primermatch[i],R1reads_w_primermatch[i], mutantampliconlength, cushionvalue)
...
#    slidedata[0]=loosematch(read1rev,read2,len(read1rev)/10)
    ...
    for i in range(0,len(read1rev)-17): # Chenged from 20 to reduce hyb failures
        
        slidepos=loosematch(read1rev[0:len(read1rev)-i], read2[i:], len(read2[i:])/20)
        if slidepos:
            #print i
            #print read1rev[0:len(read1rev)-i]
            #print read2[i:]    
            #raw_input()
            return i
...
def hybridize(read1, read2, ampliconlength, cushion):
    allowedMismatches = (int)(0.1*len(read1))  # Number of allowed mismatches should depend on length of sequences being compared, not the total length of the seqeunce
    ...
    if len(read1) == len(read2):        
        ...
        if(ampliconlength>len(read1)):
            ...
            for i in range(0,int(cushion)+1):
                #print "hyb i", i
                #print "r2", read2[offset-i:]#
                #print "r1", read1rev
                #print "am", allowedMismatches
                allowedMismatches = (int)(0.1*len(read2[offset-i:]))
        ...
        # Amplicon is shorter than read - we are reading adapter sequence
            ...
            raw_input("how did I get here?")
            offset=len(read1)-ampliconlength
            for i in range(0,int(cushion)+1):
                allowedMismatches = (int)(0.1*len(read1[offset-i:]))
